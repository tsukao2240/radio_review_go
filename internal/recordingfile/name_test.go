...
func TestBaseName(t *testing.T) {
	...
	want := "2026-06-05_0100_TBS_マヂカルラブリーのオールナイトニッポン0.aac"
	if got != want {
...
func TestBaseNameAfter24Hour(t *testing.T) {
	...
	want := "2026-06-06_0100_TBS_深夜番組.aac"
	if got != want {
		t.Fatalf("BaseName = %q, want %q", got, want)
	}
}
...
func TestBaseNameSanitizesForbiddenCharacters(t *testing.T) {
	...
	if !strings.Contains(got, "TBS_badtitle.aac") {
...
func TestNewPathAddsShortIDOnDuplicate(t *testing.T) {
	dir := t.TempDir()
	first := filepath.Join(dir, "2026-06-05_0100_TBS_Show.aac")
	if err := os.WriteFile(first, []byte("exists"), 0600); err != nil {
		t.Fatal(err)
	}
	got := NewPath(dir, "1234567890abcdef", "20260605010000", "TBS", "Show")
	want := filepath.Join(dir, "2026-06-05_0100_TBS_Show_90abcdef.aac")
	if got != want {
